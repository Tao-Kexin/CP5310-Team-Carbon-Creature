...
(function(factory) {
  ...
  var dialogDef = {
    id: '', // If there are multiple styles of pop-ups in the page, the styles can be distinguished by ID.
    title: 'Title',
    isFixed: true,
    hideHeader: false,
    hideClose: false,
    content: null,
    callback: null,
    withNoPadding: false, //  want to set padding?
    withNoMinWidth: false, //  set the minimum width?
    bgHide: true, // Click whether the background is hidden.
    escHide: true // Press ESC to hide?
  };
  ...
  var Dialog = {
    ...
    show: function(opts) {
      ...
      // Callback function executed when initializing.
      typeof dialogOpts.onInit === 'function' && dialogOpts.onInit();

      //Change the position of the content display box dynamically when the browser is resized.
    ...
    /**
     * Close the dialog pop-up window.
    ...
    install: function(opts) {
      ...
      // If the pop-up window was opened before, close it first.
  ...
  // Package of pop-up common processing function
  $.extend({
    ...
    /**
     * Prompt box for sending warning (top)
     */
  ...
  (function() {
    ...
    // Binding: used to close the dialog pop-up.
    $doc.on('click', closeClass, function() {
      var $that = $(this), beforeReturn;

      // If false is returned, the pop-up window is interrupted.
    ...
    // Binding: used to perform pop-up confirmation.
    $doc.on('click', '.j_dialogConfirm', function() {
        var $that = $(this), beforeReturn;

      //If the submit button has noconfirm attribute, the response will not be executed.
    ...
    // Bind: used to cancel the pop-up window.
    ...
    // Binding: Click the pop-up mask layer to close the pop-up.
    $doc.on('click', '.j_bgHide', function() {
      $(closeClass).trigger('click');
    });

    // Binding: Press ESC to close the pop-up.
